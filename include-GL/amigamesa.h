...
struct amigamesa_context
{
	...
	long*           ColorTable;             /* LUT8 display: ARGB -> real pen conversion */
						/* ARGB display: ARGB -> include-GL pen conversion */
	long*           ColorTable2;            /* LUT8 display: table of allocated pens */
						/* ARGB display: include-GL pen -> ARGB conversion */
	UBYTE penconv[256];                     /* pen conversion from include-GL color to real color */
	UBYTE penconvinv[256];                  /* pen conversion from real color to include-GL color */
	UBYTE dtable[256];
	UBYTE bugfix[256];
	...
	int drawbufferflag;                     /* include-GL double buffering support  */
	int readbufferflag;                     /*              "               */
	int backarrayflag;                      /*              "               */
	UBYTE* DrawBuffer;                      /*              "               */
	UBYTE* ReadBuffer;                      /*              "               */
...
AMA_TwoBuffers: allocates two back buffers in double-buffered window mode
		(or AGA fullscreen mode). This is absolutely required, if the include-GL
...
AMA_VeryFast:   Tries to render as fast as possible and doesn't care at all
		...
		percent include-GL compliant results, you should disable 3D hardware
